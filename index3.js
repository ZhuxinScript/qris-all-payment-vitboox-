    ...
    async function checkInitialFiles() {
        try {
            ...
            if (!fs.existsSync(SAVE_FILE)) {
                ...
                console.log("✅ Sync Success - File lama disimpan di sent_files")
            } else {
    ...
    sock.ev.on(
        ...
        async (update) => {
            ...
            if (connection === "close") {
                ...
                // Sistem Auto Fix Bad MAC / Session Corrupt bawaan
                if (statusCode === DisconnectReason.loggedOut || errMsg.includes("Bad MAC")) {
                    console.log("⚠️ Sesi korup / Bad MAC terdeteksi. Mereset folder auth_info...")
                    try {
        ...
        async ({ messages }) => {
            ...
            switch (cmd) {
                ...
                case "-getscript":
                    ...
                    try {
                        ...
                        if (lastFileList[no - 1]) {
                            await sock.sendMessage(
                                jid,
                                { text: `loadstring(game:HttpGet("\${RAW_BASE + lastFileList[no - 1]}"))()` },
                                { quoted: msg }
                            )
                            await sock.sendMessage(jid, { react: { text: "✅", key: msg.key } }).catch(() => {})
                        } else {
                ...
                case "-item":
                    ...
                    try {
                        // Menggunakan API Resmi Catalog V1 (POST) yang kebal block / kebal error private palsu di hostingan panel
                        ...
                        // Fallback jika data tidak terbaca di API catalog utama (misal tipe non-asset)
                        if (!itemData) {
